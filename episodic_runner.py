...
def fetch_daily_bars(ticker: str, n_days: int) -> pd.DataFrame:
    ...
    req = StockBarsRequest(
        ...
        feed=DataFeed.IEX,  # free-tier: IEX feed (SIP requires paid subscription)
    )
...
def run_midday() -> None:
    ...
    engine, features = load_or_train_hmm(n_bars=300)  # sma200(min_p=100)+zscore(min_p=63)→163 min
    regime = predict_regime(engine, features)
    is_high_vol = regime.label in HIGH_VOL_LABELS
