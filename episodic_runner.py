...
def run_market_open() -> None:
    ...
    # Fetch bars for all ETFs so strategies can compute ATR/signals
    all_bars: dict[str, pd.DataFrame] = {}
    for ticker in settings.TICKERS:
        try:
            all_bars[ticker] = fetch_daily_bars(ticker, 300)
        except Exception as exc:
            logger.warning("Could not fetch bars for %s: %s — skipping", ticker, exc)
