...
def fetch_daily_bars(ticker: str, n_days: int) -> pd.DataFrame:
    ...
    if _feed is not None:
        _req_kwargs["feed"] = _feed
    req = _SBR(**_req_kwargs)
...
def run_market_open() -> None:
    ...
    for ticker in settings.TICKERS:
        bars_dict[ticker] = fetch_daily_bars(ticker, 400)

    is_flickering = not regime.is_confirmed or regime.consecutive_bars < 3
    signals = orchestrator.generate_signals(
        symbols=list(settings.TICKERS),
        bars=bars_dict,
        regime_state=regime,
