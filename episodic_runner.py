...
def fetch_daily_bars(ticker: str, n_days: int) -> pd.DataFrame:
    ...
    try:
        ...
        msg = str(exc).lower()
        if "subscription" in msg or "forbidden" in msg or "403" in msg:
            logger.warning("Alpaca SIP data blocked (%s) — falling back to yfinance", exc)
...
def _fetch_daily_bars_alpaca(ticker: str, n_days: int) -> pd.DataFrame:
    ...
    logger.info("Fetched %d bars for %s (Alpaca)", len(df), ticker)
    return df
...
def _fetch_daily_bars_yfinance(ticker: str, n_days: int) -> pd.DataFrame:
    ...
    # Fetch extra calendar days to cover weekends/holidays
