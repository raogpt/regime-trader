...
def fetch_daily_bars(ticker: str, n_days: int) -> pd.DataFrame:
    ...
    def _fetch(feed=None):
        return client.get_stock_bars(_make_req(feed)).df

    # Try SIP first, fall back to IEX (free, ~15-min delayed) on subscription error
    try:
        raw = with_retry(lambda: _fetch())
    except Exception as e:
        if "subscription" in str(e).lower() or "403" in str(e):
            logger.warning("SIP feed unavailable (%s) — falling back to IEX", e)
            raw = with_retry(lambda: _fetch(DataFeed.IEX))
        else:
            raise
