...
def run_market_open() -> None:
    ...
    all_regime_infos = [engine.get_regime_info(label) for label in engine.regime_labels]
    orchestrator = StrategyOrchestrator(all_regime_infos)

    ticker_bars = {t: fetch_daily_bars(t, 60) for t in settings.TICKERS}
    ...
    for signal in signals:
        ...
        try:
            results = order_executor.rebalance(signal, account_value)
            for r in results:
