...
def run_market_open() -> None:
    ...
    cb_result = risk_manager.update_portfolio_value(float(acct.equity), str(regime.label))
    ...
    all_regime_infos = [engine.get_regime_info(l) for l in engine.regime_labels]
    orchestrator = StrategyOrchestrator(all_regime_infos)
    ticker_bars = {t: fetch_daily_bars(t, 100) for t in settings.TICKERS}
