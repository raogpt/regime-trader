...
def run_market_open() -> None:
    ...
    # Build RegimeInfo list for all labels and construct orchestrator
    ...
    # Fetch OHLCV bars for each ETF (need ~50 bars for strategy logic)
    etf_bars: dict[str, pd.DataFrame] = {}
    for ticker in settings.TICKERS:
        try:
            etf_bars[ticker] = fetch_daily_bars(ticker, 100)
        except Exception as exc:
    ...
    # Apply cross-enrichment sizing modifier to all signals
    for sig in signals:
        sig.position_size_pct *= sizing_modifier
