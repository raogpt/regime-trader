...
def load_or_train_hmm(n_bars: int = 300) -> tuple[HMMEngine, np.ndarray]:
    """
...
def run_market_open() -> None:
    ...
    if not regime.is_confirmed:
        logger.info("Regime not stable (consecutive_bars=%d < 3) → HOLD", regime.consecutive_bars)
        append_to_log(TRADE_LOG,
                      f"\n### {date.today().isoformat()} market-open — HOLD (regime unstable, bars={regime.consecutive_bars})\n")
        git_commit_memory("market-open")
        return
    ...
    # Fetch OHLCV bars for each ticker for signal generation
    ticker_bars: dict[str, pd.DataFrame] = {}
    for tkr in settings.TICKERS:
        try:
            ticker_bars[tkr] = fetch_daily_bars(tkr, n_days=300)
        except Exception as exc:
