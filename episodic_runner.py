...
def fetch_daily_bars(ticker: str, n_days: int) -> pd.DataFrame:
    ...
    def _fetch():
        # Try SIP first; fall back to IEX for free-tier accounts
        try:
            return client.get_stock_bars(_build_req()).df
        except Exception as sip_err:
            if "subscription" in str(sip_err).lower() or "403" in str(sip_err):
                logger.warning("SIP feed unavailable (%s) — retrying with IEX", sip_err)
                return client.get_stock_bars(_build_req(feed=DataFeed.IEX)).df
            raise
...
def retrain_hmm_full() -> tuple[HMMEngine, np.ndarray]:
    ...
    features = engineer.build_feature_dataframe(bars).values
