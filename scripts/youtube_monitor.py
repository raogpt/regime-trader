...
try:
    import feedparser
    # Verify feedparser is actually importable (Python 3.11 compat check)
    _ = feedparser.parse
except Exception:
    log.warning("feedparser unavailable — using stdlib xml.etree fallback")
...
def _parse_rss_elementtree(xml_text: str) -> list[dict]:
    ...
    for entry in root.findall("atom:entry", NS):
        ...
        if pub_el is not None and pub_el.text:
            try:
                ...
                published_dt = datetime.fromisoformat(raw).astimezone(timezone.utc)
            except Exception:
                pass
...
def fetch_rss(channel_id: str) -> list[dict]:
    ...
    if feedparser is not None:
        ...
        try:
            entries = _parse_rss_elementtree(resp.text)
        except Exception as exc:
            log.warning("ElementTree RSS parse failed for %s: %s", channel_id, exc)
            return []
