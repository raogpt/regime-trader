...
try:
    import feedparser
    # Verify it actually works (sgmllib3k dependency may be missing)
    feedparser.parse  # noqa: B018
except (ImportError, AttributeError, Exception):
    log.warning("feedparser unavailable — will use stdlib xml fallback")
...
def fetch_rss(channel_id: str) -> list[dict]:
    ...
    Uses feedparser when available, falls back to stdlib xml.etree.
    """
    if requests is None:
    ...
    if feedparser is not None:
        ...
        log.info("  (using stdlib xml fallback for RSS parsing)")
