...
try:
    ...
    log.warning("feedparser unavailable — will use stdlib XML parser fallback")
...
def _parse_rss_stdlib(xml_text: str) -> list[dict]:
    """
    Fallback Atom/RSS parser using stdlib xml.etree.ElementTree.
    Handles YouTube Atom feeds (yt:videoId namespace).
    """
    ...
    for entry in root.findall("atom:entry", NS):
        # Video ID: prefer yt:videoId element, fallback to link href
        ...
        if pub_el is not None and pub_el.text:
            ...
            for fmt in ("%Y-%m-%dT%H:%M:%S%z", "%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%d"):
                try:
                    published_dt = datetime.strptime(raw, fmt)
...
def fetch_rss(channel_id: str) -> list[dict]:
    ...
    # Use feedparser when available; fall back to stdlib XML parser
    if feedparser is not None:
        ...
        log.info("feedparser unavailable — using stdlib XML parser")
        entries = _parse_rss_stdlib(resp.text)
